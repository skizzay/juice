    ...
    static ValueValidator matches(Pattern pattern) {
        return value -> {
            if (null == value || !pattern.matcher(value).matches()) {
                throw new IllegalArgumentException(String.format("Value[%s] must match pattern[%s]", value, pattern));
            }
        };
    }

    static ValueValidator equalTo(String expected) {
        return value -> {
            if (!expected.equals(value)) {
                throw new IllegalArgumentException(String.format("Value[%s] must equal %s", value, expected));
            }
        };
    }

    static ValueValidator equalToIgnoreCase(String expected) {
        return value -> {
            if (!expected.equalsIgnoreCase(value)) {
                throw new IllegalArgumentException(String.format("Value[%s] must equal (ignoring case) %s", value, expected));
            }
        };
    }

    static ValueValidator notEqualTo(String expected) {
        return value -> {
            if (expected.equals(value)) {
                throw new IllegalArgumentException(String.format("Value[%s] must not equal %s", value, expected));
            }
        };
    }

    static ValueValidator notEqualToIgnoreCase(String expected) {
        return value -> {
            if (expected.equalsIgnoreCase(value)) {
                throw new IllegalArgumentException(String.format("Value[%s] must not equal (ignoring case) %s", value, expected));
            }
        };
    }
}
