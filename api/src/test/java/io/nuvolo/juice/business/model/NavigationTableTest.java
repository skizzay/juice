...
class NavigationTableTest {
    ...
    void getNavigations_withNavigations_returnsNavigations() {
        // Arrange
        final NavigationTable target = createTarget();
        target.addNavigation(new Navigation(ScreenName.of("source1"), ScreenName.of("target1"), screen -> {}));
        target.addNavigation(new Navigation(ScreenName.of("source2"), ScreenName.of("target2"), screen -> {}));
    ...
    void addNavigation_givenDuplicate_throws() {
        // Arrange
        final NavigationTable target = createTarget();
        final var navigation = new Navigation(ScreenName.of("source"), ScreenName.of("target"), screen -> {});
        target.addNavigation(navigation);

        // Act
        final var exception = assertThrows(IllegalArgumentException.class, () -> target.addNavigation(navigation));

        // Assert
        assertEquals("Navigation already exists from 'source' to 'target'", exception.getMessage());
    }
    ...
    void getNavigationAction_givenUnknownSource_isEmpty() {
        // Arrange
        final NavigationTable target = createTarget();
        target.addNavigation(new Navigation(ScreenName.of("source1"), ScreenName.of("target"), screen -> {}));
